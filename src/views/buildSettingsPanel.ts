...
<div class="grid">
<section class="card wide"><details open><summary>Target</summary><div class="section-body"><label class="field">Target type<select id="targetType">${targetOption('Executable', target.targetType)}${targetOption('Dynamic Link Library', target.targetType)}${targetOption('Static Library', target.targetType)}</select></label>${pathField('Output file', 'outputPath', target.outputPath)}<p class="muted">The output path applies to ${escapeHtml(scopeLabel(mode))}.</p></div></details></section>
...
const renderForcedModules=()=>{forcedModules=[...new Set(forcedModules.map(v=>String(v).trim()).filter(Boolean))];if(el('forcedModules'))el('forcedModules').value=forcedModules.join('\\n');const host=el('forcedModulesPreview');if(!host)return;host.innerHTML='';if(!forcedModules.length){const empty=document.createElement('div');empty.className='empty-modules';empty.textContent='No module is currently included.';host.appendChild(empty);updateForcedModuleControls();return;}forcedModules.forEach((value,index)=>{const row=document.createElement('div');row.className='module-row';const label=document.createElement('span');label.textContent=value;label.title=value;const remove=document.createElement('button');remove.type='button';remove.className='module-remove';remove.textContent='Remove';remove.addEventListener('click',()=>{forcedModules.splice(index,1);renderForcedModules();});row.append(label,remove);host.appendChild(row);});updateForcedModuleControls();};
el('targetType').addEventListener('change',()=>{document.body.dataset.target=val('targetType');replaceOptions('runtimeBinding',bindingOptions[val('targetType')]||[]);updateForcedModuleControls();updateTargetCreationControls();updateDllOptionControls();updateExecutableCommandLineControls();});
...
el('runtimeSupport')?.addEventListener('change',updateForcedModuleControls);
el('manifestEmbed')?.addEventListener('change',updateTargetCreationControls);
el('useDefaultImportLibBaseName')?.addEventListener('change',updateDllOptionControls);
el('whereToCopyDll')?.addEventListener('change',updateDllOptionControls);
el('dllExports')?.addEventListener('change',updateDllOptionControls);
el('openImportLibraryChoices')?.addEventListener('click',showImportLibraryChoices);
el('confirmImportLibraryChoices')?.addEventListener('click',()=>hideImportLibraryChoices(false));
el('cancelImportLibraryChoices')?.addEventListener('click',()=>hideImportLibraryChoices(true));
...
el('usingLoadExternalModule')?.addEventListener('change',updateForcedModuleControls);
el('addTypeLibToDll')?.addEventListener('change',updateDllTypeInformationControls);
el('includeTypeLibHelpLinks')?.addEventListener('change',updateDllTypeInformationControls);
...
el('signEnabled')?.addEventListener('change',updateSigningControls);
el('addForcedModules')?.addEventListener('click',()=>vscode.postMessage({type:'browseForcedModules'}));
el('addForcedModuleName')?.addEventListener('click',()=>vscode.postMessage({type:'promptForcedModuleName'}));
window.addEventListener('message',(event)=>{const message=event.data;if(message?.type==='setField'&&el(message.field))el(message.field).value=message.value||'';if(message?.type==='appendForcedModules'&&Array.isArray(message.values)){forcedModules.push(...message.values);renderForcedModules();}});
renderForcedModules();updateForcedModuleControls();updateTargetCreationControls();updateDllOptionControls();updateDllTypeInformationControls();updateSigningControls();updateExecutableCommandLineControls();
