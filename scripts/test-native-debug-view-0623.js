...
const vsix = '/mnt/data/labwindows-cvi-project-manager-0.6.23.vsix';

assert.strictEqual(pkg.version, '0.6.23');
for (const token of [
...
assert(outView.includes("description: enabled ? undefined : 'Unavailable'"));
assert(outView.includes("icon: enabled ? icon : 'circle-slash'"));
...
const result = {
  ...
  buildAndRunDisabledDuringExecution: true,
  pauseEnabledForCachedRunningState: true,
  continueAndStopRemainAvailableForActivePersistentSession: true,
  asynchronousBreakpointSuspensionCovered: true,
  noDashboardWebview: true,
  packagedCompiledDashboard: true
};
fs.writeFileSync(path.join(root, 'NATIVE_DEBUG_DASHBOARD_0.6.23_VALIDATION.json'), JSON.stringify(result, null, 2) + '\n');
